...
namespace coreneuron {
    ...
    static inline int get_mech_type() {
        return heat_eqn_global_global.mech_type;
    }
    ...
    static void nrn_private_constructor_heat_eqn_global(NrnThread* nt, Memb_list* ml, int type) {
        assert(!ml->instance);
        assert(!ml->global_variables);
        assert(ml->global_variables_size == 0);
        auto* const inst = new heat_eqn_global_Instance{};
        assert(inst->global == &heat_eqn_global_global);
        ml->instance = inst;
        ml->global_variables = inst->global;
        ml->global_variables_size = sizeof(heat_eqn_global_Store);
    }

    // Deallocate the instance structure
    static void nrn_private_destructor_heat_eqn_global(NrnThread* nt, Memb_list* ml, int type) {
        auto* const inst = static_cast<heat_eqn_global_Instance*>(ml->instance);
        assert(inst);
        assert(inst->global);
        assert(inst->global == &heat_eqn_global_global);
        assert(inst->global == ml->global_variables);
        assert(ml->global_variables_size == sizeof(heat_eqn_global_Store));
        delete inst;
        ml->instance = nullptr;
    ...
    static void nrn_alloc_heat_eqn_global(double* data, Datum* indexes, int type) {
        // do nothing
    }


    void nrn_constructor_heat_eqn_global(NrnThread* nt, Memb_list* ml, int type) {
    ...
    void nrn_destructor_heat_eqn_global(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
    ...
    struct functor_heat_eqn_global_0 {
        ...
        void initialize() {
            inst->global->muinst->X[id];
            source0_ = 0.0;
            old_X = inst->X[id];
        }

        functor_heat_eqn_global_0(NrnThread* nt, heat_eqn_global_Instance* inst, int id, int pnodecount, double v, const Datum* indexes, double* data, ThreadDatum* thread)
    ...
    /** initialize channel */
    void nrn_init_heat_eqn_global(NrnThread* nt, Memb_list* ml, int type) {
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
        double* data = ml->data;
    ...
    /** update state */
    void nrn_state_heat_eqn_global(NrnThread* nt, Memb_list* ml, int type) {
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
        double* data = ml->data;
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            // call newton solver
            functor_heat_eqn_global_0 newton_functor(nt, inst, id, pnodecount, v, indexes, data, thread);
    ...
    void _heat_eqn_global_reg() {

        int mech_type = nrn_get_mechtype("heat_eqn_global");
        heat_eqn_global_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }

        _nrn_layout_reg(mech_type, 0);
        register_mech(mechanism_info, nrn_alloc_heat_eqn_global, nullptr, nullptr, nrn_state_heat_eqn_global, nrn_init_heat_eqn_global, nrn_private_constructor_heat_eqn_global, nrn_private_destructor_heat_eqn_global, first_pointer_var_index(), 1);
