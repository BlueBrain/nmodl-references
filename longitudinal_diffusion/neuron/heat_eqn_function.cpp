...
namespace neuron {
    ...
    static constexpr int ode_count_heat_eqn_function(int _type) {
        return 1;
    }


    static int ode_update_nonstiff_heat_eqn_function(_nrn_mechanism_cache_range& _lmc, heat_eqn_function_Instance& inst, heat_eqn_function_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt, heat_eqn_function_ThreadVariables& _thread_vars) {
        auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
        double source0_;
        ;
        source0_ = 0.0;
        inst.DX[id] = ((source0_)) / (vol_heat_eqn_function(_lmc, inst, node_data, id, _ppvar, _thread, _thread_vars, nt, inst.x[id]));
        return 0;
    }


    static void ode_setup_nonstiff_heat_eqn_function(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
            ode_update_nonstiff_heat_eqn_function(_lmc, inst, node_data, id, _ppvar, _thread, nt, _thread_vars);
        }
    }


    static void ode_setup_tolerances_heat_eqn_function(Prop* _prop, int equation_index, neuron::container::data_handle<double>* _pv, neuron::container::data_handle<double>* _pvdot, double* _atol, int _type) {
        auto* _ppvar = _nrn_mechanism_access_dparam(_prop);
        _ppvar[0].literal_value<int>() = equation_index;
        for (int i = 0; i < ode_count_heat_eqn_function(0); i++) {
    ...
    static void ode_update_stiff_heat_eqn_function(_nrn_mechanism_cache_range& _lmc, heat_eqn_function_Instance& inst, heat_eqn_function_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt, heat_eqn_function_ThreadVariables& _thread_vars) {
        double source0_;
        ;
        source0_ = 0.0;
    ...
    static void ode_setup_stiff_heat_eqn_function(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        _nrn_mechanism_cache_range _lmc{_sorted_token, *nt, *_ml_arg, _type};
        auto inst = make_instance_heat_eqn_function(&_lmc);
        auto nodecount = _ml_arg->nodecount;
        auto node_data = make_node_data_heat_eqn_function(*nt, *_ml_arg);
        ...
        for (int id = 0; id < nodecount; id++) {
            auto* _ppvar = _ml_arg->pdata[id];
            auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
            ode_update_stiff_heat_eqn_function(_lmc, inst, node_data, id, _ppvar, _thread, nt, _thread_vars);
        }
    }
    ...
    struct functor_heat_eqn_function_0 {
        ...
        functor_heat_eqn_function_0(_nrn_mechanism_cache_range& _lmc, heat_eqn_function_Instance& inst, heat_eqn_function_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, heat_eqn_function_ThreadVariables& _thread_vars, NrnThread* nt, double v)
    ...
    static void nrn_state_heat_eqn_function(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            // call newton solver
            functor_heat_eqn_function_0 newton_functor(_lmc, inst, node_data, id, _ppvar, _thread, _thread_vars, nt, v);
    ...
    extern "C" void _heat_eqn_function_reg() {
        ...
        hoc_register_cvode(mech_type, ode_count_heat_eqn_function, ode_setup_tolerances_heat_eqn_function, ode_setup_nonstiff_heat_eqn_function, ode_setup_stiff_heat_eqn_function);
