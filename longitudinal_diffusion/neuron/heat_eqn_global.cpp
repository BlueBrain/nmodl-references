/*********************************************************
Model Name      : heat_eqn_global
Filename        : heat_eqn_global.mod
NMODL Version   : 7.7.0
Vectorized      : true
Threadsafe      : true
Created         : DATE
...
namespace neuron {
    ...
    struct heat_eqn_global_Store {
        double mu{2};
        double vol{0.01};
        double X0{0};
    };
    static_assert(std::is_trivially_copy_constructible_v<heat_eqn_global_Store>);
    static_assert(std::is_trivially_move_constructible_v<heat_eqn_global_Store>);
    static_assert(std::is_trivially_copy_assignable_v<heat_eqn_global_Store>);
    static_assert(std::is_trivially_move_assignable_v<heat_eqn_global_Store>);
    static_assert(std::is_trivially_destructible_v<heat_eqn_global_Store>);
    static heat_eqn_global_Store heat_eqn_global_global;
    auto mu_heat_eqn_global() -> std::decay<decltype(heat_eqn_global_global.mu)>::type  {
        return heat_eqn_global_global.mu;
    }
    auto vol_heat_eqn_global() -> std::decay<decltype(heat_eqn_global_global.vol)>::type  {
        return heat_eqn_global_global.vol;
    }
    auto X0_heat_eqn_global() -> std::decay<decltype(heat_eqn_global_global.X0)>::type  {
        return heat_eqn_global_global.X0;
    }
    ...
    struct heat_eqn_global_Instance  {
        double* x{};
        double* X{};
        double* DX{};
        double* v_unused{};
        double* g_unused{};
        heat_eqn_global_Store* global{&heat_eqn_global_global};
    };


    struct heat_eqn_global_NodeData  {
    ...
    static heat_eqn_global_Instance make_instance_heat_eqn_global(_nrn_mechanism_cache_range* _lmc) {
        if(_lmc == nullptr) {
            return heat_eqn_global_Instance();
        }
    ...
    static heat_eqn_global_NodeData make_node_data_heat_eqn_global(NrnThread& nt, Memb_list& _ml_arg) {
        return heat_eqn_global_NodeData {
            _ml_arg.nodeindices,
    ...
    static heat_eqn_global_NodeData make_node_data_heat_eqn_global(Prop * _prop) {
        if(!_prop) {
            return heat_eqn_global_NodeData();
        }
    ...
    static void nrn_destructor_heat_eqn_global(Prop* prop);


    static void nrn_alloc_heat_eqn_global(Prop* _prop) {
    ...
    static double _diffusion_coefficient_X(int _i, Memb_list* _ml_arg, size_t id, Datum* _ppvar, double* _pdvol, double* _pdfcdc, Datum* /* _thread */, NrnThread* nt, const _nrn_model_sorted_token& _sorted_token) {
        ...
        auto inst = make_instance_heat_eqn_global(&_lmc);
        auto node_data = make_node_data_heat_eqn_global(*nt, *_ml_arg);
        auto* _thread = _ml_arg->_thread;
        *_pdvol= inst.global->vol;
        *_pdfcdc = 0.0;
        return inst.global->mu;
    }
    ...
    static constexpr int ode_count_heat_eqn_global(int _type) {
        return 1;
    }


    static int ode_update_nonstiff_heat_eqn_global(_nrn_mechanism_cache_range& _lmc, heat_eqn_global_Instance& inst, heat_eqn_global_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt) {
        auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
        double source0_;
        ;
        source0_ = 0.0;
        inst.DX[id] = ((source0_)) / (inst.global->vol);
        return 0;
    }


    static void ode_setup_nonstiff_heat_eqn_global(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
            ode_update_nonstiff_heat_eqn_global(_lmc, inst, node_data, id, _ppvar, _thread, nt);
        }
    }


    static void ode_setup_tolerances_heat_eqn_global(Prop* _prop, int equation_index, neuron::container::data_handle<double>* _pv, neuron::container::data_handle<double>* _pvdot, double* _atol, int _type) {
        auto* _ppvar = _nrn_mechanism_access_dparam(_prop);
        _ppvar[0].literal_value<int>() = equation_index;
        for (int i = 0; i < ode_count_heat_eqn_global(0); i++) {
    ...
    static void ode_update_stiff_heat_eqn_global(_nrn_mechanism_cache_range& _lmc, heat_eqn_global_Instance& inst, heat_eqn_global_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt) {
        auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
        double source0_;
        ;
        source0_ = 0.0;
    ...
    static void ode_setup_stiff_heat_eqn_global(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        _nrn_mechanism_cache_range _lmc{_sorted_token, *nt, *_ml_arg, _type};
        auto inst = make_instance_heat_eqn_global(&_lmc);
        auto nodecount = _ml_arg->nodecount;
        auto node_data = make_node_data_heat_eqn_global(*nt, *_ml_arg);
        ...
        for (int id = 0; id < nodecount; id++) {
            auto* _ppvar = _ml_arg->pdata[id];
            auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
            ode_update_stiff_heat_eqn_global(_lmc, inst, node_data, id, _ppvar, _thread, nt);
        }
    }
    ...
    struct functor_heat_eqn_global_0 {
        _nrn_mechanism_cache_range& _lmc;
        heat_eqn_global_Instance& inst;
        heat_eqn_global_NodeData& node_data;
        size_t id;
        Datum* _ppvar;
        Datum* _thread;
        NrnThread* nt;
        ...
        functor_heat_eqn_global_0(_nrn_mechanism_cache_range& _lmc, heat_eqn_global_Instance& inst, heat_eqn_global_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt, double v)
        ...
        void operator()(const Eigen::Matrix<double, 1, 1>& nmodl_eigen_xm, Eigen::Matrix<double, 1, 1>& nmodl_eigen_dxm, Eigen::Matrix<double, 1, 1>& nmodl_eigen_fm, Eigen::Matrix<double, 1, 1>& nmodl_eigen_jm) const {
            ...
            nmodl_eigen_f[static_cast<int>(0)] =  -nmodl_eigen_x[static_cast<int>(0)] / nt->_dt + source0_ / inst.global->vol + old_X / nt->_dt;
    ...
    static DoubScal hoc_scalar_double[] = {
        {"mu_heat_eqn_global", &heat_eqn_global_global.mu},
        {"vol_heat_eqn_global", &heat_eqn_global_global.vol},
        {nullptr, nullptr}
    };
    ...
    static VoidFunc hoc_intfunc[] = {
        {"setdata_heat_eqn_global", _hoc_setdata},
        {nullptr, nullptr}
    };
    ...
    static void nrn_init_heat_eqn_global(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        _nrn_mechanism_cache_range _lmc{_sorted_token, *nt, *_ml_arg, _ml_arg->type()};
        auto inst = make_instance_heat_eqn_global(&_lmc);
        auto node_data = make_node_data_heat_eqn_global(*nt, *_ml_arg);
        auto* _thread = _ml_arg->_thread;
    ...
    static void nrn_state_heat_eqn_global(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        _nrn_mechanism_cache_range _lmc{_sorted_token, *nt, *_ml_arg, _ml_arg->type()};
        auto inst = make_instance_heat_eqn_global(&_lmc);
        auto node_data = make_node_data_heat_eqn_global(*nt, *_ml_arg);
        auto* _thread = _ml_arg->_thread;
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            // call newton solver
            functor_heat_eqn_global_0 newton_functor(_lmc, inst, node_data, id, _ppvar, _thread, nt, v);
    ...
    static void nrn_jacob_heat_eqn_global(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        _nrn_mechanism_cache_range _lmc{_sorted_token, *nt, *_ml_arg, _ml_arg->type()};
        auto inst = make_instance_heat_eqn_global(&_lmc);
        auto node_data = make_node_data_heat_eqn_global(*nt, *_ml_arg);
        auto* _thread = _ml_arg->_thread;
    ...
    static void nrn_destructor_heat_eqn_global(Prop* prop) {
        ...
        const size_t id = 0;
        auto inst = make_instance_heat_eqn_global(prop ? &_lmc : nullptr);
        auto node_data = make_node_data_heat_eqn_global(prop);
    ...
    extern "C" void _heat_eqn_global_reg() {
        _initlists();

        register_mech(mechanism_info, nrn_alloc_heat_eqn_global, nullptr, nrn_jacob_heat_eqn_global, nrn_state_heat_eqn_global, nrn_init_heat_eqn_global, -1, 1);
        ...
        hoc_register_cvode(mech_type, ode_count_heat_eqn_global, ode_setup_tolerances_heat_eqn_global, ode_setup_nonstiff_heat_eqn_global, ode_setup_stiff_heat_eqn_global);
