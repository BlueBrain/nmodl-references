...
namespace neuron {
    ...
    static constexpr int ode_count_heat_eqn_thread_vars(int _type) {
        return 1;
    }
    ...
    static int ode_update_nonstiff_heat_eqn_thread_vars(_nrn_mechanism_cache_range& _lmc, heat_eqn_thread_vars_Instance& inst, heat_eqn_thread_vars_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt, heat_eqn_thread_vars_ThreadVariables& _thread_vars) {
        ...
        ;
        source0_ = 0.0;
        inst.DX[id] = ((source0_)) / (_thread_vars.vol(id));
        return 0;
    }
    ...
    static void ode_update_stiff_heat_eqn_thread_vars(_nrn_mechanism_cache_range& _lmc, heat_eqn_thread_vars_Instance& inst, heat_eqn_thread_vars_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt, heat_eqn_thread_vars_ThreadVariables& _thread_vars) {
        double source0_;
        ;
        source0_ = 0.0;
    ...
    struct functor_heat_eqn_thread_vars_0 {
        ...
        functor_heat_eqn_thread_vars_0(_nrn_mechanism_cache_range& _lmc, heat_eqn_thread_vars_Instance& inst, heat_eqn_thread_vars_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, heat_eqn_thread_vars_ThreadVariables& _thread_vars, NrnThread* nt, double v)
    ...
    void nrn_state_heat_eqn_thread_vars(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            // call newton solver
            functor_heat_eqn_thread_vars_0 newton_functor(_lmc, inst, node_data, id, _ppvar, _thread, _thread_vars, nt, v);
