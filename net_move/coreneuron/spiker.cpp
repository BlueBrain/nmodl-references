    ...
    static inline void net_receive_kernel_spiker(double t, Point_process* pnt, spiker_Instance* inst, NrnThread* nt, Memb_list* ml, int weight_index, double flag) {
        ...
        inst->tsave[id] = t;
        {
            if (flag == 0.0) {
                ...
                net_send_buffering(nt, ml->_net_send_buffer, 2, inst->tqitem[2*pnodecount+id], -1, inst->point_process[1*pnodecount+id], t + 0.1, 0.0);
            } else {
                inst->z[id] = inst->z[id] + 1.0;
                net_send_buffering(nt, ml->_net_send_buffer, 0, inst->tqitem[2*pnodecount+id], weight_index, inst->point_process[1*pnodecount+id], t+2.0, 1.0);
            }
        }
    }
    ...
    void nrn_init_spiker(NrnThread* nt, Memb_list* ml, int type) {
        ...
        if (_nrn_skip_initmodel == 0) {
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                inst->y[id] = 0.0;
                inst->z[id] = 0.0;
                net_send_buffering(nt, ml->_net_send_buffer, 0, inst->tqitem[2*pnodecount+id], 0, inst->point_process[1*pnodecount+id], nt->_t+1.8, 1.0);
            }
        }
