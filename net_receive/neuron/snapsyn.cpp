...
namespace neuron {
    ...
    inline double nrn_current_SnapSyn(_nrn_mechanism_cache_range* _ml, NrnThread* _nt, Datum* _ppvar, Datum* _thread, double* _thread_globals, size_t id, SnapSyn_Instance& inst, SnapSyn_NodeData& node_data, double v) {
        double current = 0.0;
    ...
    void nrn_cur_SnapSyn(_nrn_model_sorted_token const& _sorted_token, NrnThread* _nt, Memb_list* _ml_arg, int _type) {
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            auto* _ppvar = _ml_arg->pdata[id];
            double I1 = nrn_current_SnapSyn(_ml, _nt, _ppvar, _thread, _thread_globals, id, inst, node_data, v+0.001);
            double I0 = nrn_current_SnapSyn(_ml, _nt, _ppvar, _thread, _thread_globals, id, inst, node_data, v);
            double rhs = I0;
