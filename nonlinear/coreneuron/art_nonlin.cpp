...
namespace coreneuron {
    ...
    struct art_nonlin_Store {
        int point_type{};
        double x0{};
        int reset{};
        int mech_type{};
    };
    static_assert(std::is_trivially_copy_constructible_v<art_nonlin_Store>);
    static_assert(std::is_trivially_move_constructible_v<art_nonlin_Store>);
    ...
    static inline int get_mech_type() {
        return art_nonlin_global.mech_type;
    }
    ...
    static void nrn_private_constructor_art_nonlin(NrnThread* nt, Memb_list* ml, int type) {
        assert(!ml->instance);
        assert(!ml->global_variables);
        assert(ml->global_variables_size == 0);
        auto* const inst = new art_nonlin_Instance{};
        assert(inst->global == &art_nonlin_global);
        ml->instance = inst;
        ml->global_variables = inst->global;
        ml->global_variables_size = sizeof(art_nonlin_Store);
    }

    // Deallocate the instance structure
    static void nrn_private_destructor_art_nonlin(NrnThread* nt, Memb_list* ml, int type) {
        auto* const inst = static_cast<art_nonlin_Instance*>(ml->instance);
        assert(inst);
        assert(inst->global);
        assert(inst->global == &art_nonlin_global);
        assert(inst->global == ml->global_variables);
        assert(ml->global_variables_size == sizeof(art_nonlin_Store));
        delete inst;
        ml->instance = nullptr;
    ...
    static void nrn_alloc_art_nonlin(double* data, Datum* indexes, int type) {
        // do nothing
    }


    void nrn_constructor_art_nonlin(NrnThread* nt, Memb_list* ml, int type) {
    ...
    void nrn_destructor_art_nonlin(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
    ...
    struct functor_art_nonlin_0 {
        ...
        functor_art_nonlin_0(NrnThread* nt, art_nonlin_Instance* inst, int id, int pnodecount, double v, const Datum* indexes, double* data, ThreadDatum* thread)
    ...
    inline double solve_art_nonlin(int id, int pnodecount, art_nonlin_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        ...
        // call newton solver
        functor_art_nonlin_0 newton_functor(nt, inst, id, pnodecount, v, indexes, data, thread);
    ...
    /** initialize channel */
    void nrn_init_art_nonlin(NrnThread* nt, Memb_list* ml, int type) {
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
        double* data = ml->data;
    ...
    void _art_nonlin_reg() {

        int mech_type = nrn_get_mechtype("art_nonlin");
        art_nonlin_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }
