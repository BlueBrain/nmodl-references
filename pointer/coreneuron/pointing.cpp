...
namespace coreneuron {
    ...
    static inline int get_mech_type() {
        return pointing_global.mech_type;
    }
    ...
    static void nrn_private_constructor_pointing(NrnThread* nt, Memb_list* ml, int type) {
        assert(!ml->instance);
        assert(!ml->global_variables);
        assert(ml->global_variables_size == 0);
        auto* const inst = new pointing_Instance{};
        assert(inst->global == &pointing_global);
        ml->instance = inst;
        ml->global_variables = inst->global;
        ml->global_variables_size = sizeof(pointing_Store);
    }

    // Deallocate the instance structure
    static void nrn_private_destructor_pointing(NrnThread* nt, Memb_list* ml, int type) {
        auto* const inst = static_cast<pointing_Instance*>(ml->instance);
        assert(inst);
        assert(inst->global);
        assert(inst->global == &pointing_global);
        assert(inst->global == ml->global_variables);
        assert(ml->global_variables_size == sizeof(pointing_Store));
        delete inst;
        ml->instance = nullptr;
    ...
    static void nrn_alloc_pointing(double* data, Datum* indexes, int type) {
        // do nothing
    }


    void nrn_constructor_pointing(NrnThread* nt, Memb_list* ml, int type) {
    ...
    void nrn_destructor_pointing(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
    ...
    inline double is_valid_pointing(int id, int pnodecount, pointing_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        ...
        if (nrn_pointing(&inst->ptr[indexes[0*pnodecount + id]])) {
            ret_is_valid = 1.0;
        } else {
            ret_is_valid = 0.0;
        }
    ...
    /** initialize channel */
    void nrn_init_pointing(NrnThread* nt, Memb_list* ml, int type) {
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
        double* data = ml->data;
    ...
    void _pointing_reg() {
        ...
        int mech_type = nrn_get_mechtype("pointing");
        pointing_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }

        _nrn_layout_reg(mech_type, 0);
        register_mech(mechanism_info, nrn_alloc_pointing, nullptr, nullptr, nullptr, nrn_init_pointing, nrn_private_constructor_pointing, nrn_private_destructor_pointing, first_pointer_var_index(), 1);
