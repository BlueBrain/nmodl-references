...
namespace neuron {
    ...
    static constexpr int ode_count_cnexp_scalar(int _type) {
        return 1;
    }


    static int ode_update_nonstiff_cnexp_scalar(_nrn_mechanism_cache_range& _lmc, cnexp_scalar_Instance& inst, cnexp_scalar_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt) {
    ...
    static void ode_setup_nonstiff_cnexp_scalar(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
            ode_update_nonstiff_cnexp_scalar(_lmc, inst, node_data, id, _ppvar, _thread, nt);
        }
    }


    static void ode_setup_tolerances_cnexp_scalar(Prop* _prop, int equation_index, neuron::container::data_handle<double>* _pv, neuron::container::data_handle<double>* _pvdot, double* _atol, int _type) {
        auto* _ppvar = _nrn_mechanism_access_dparam(_prop);
        _ppvar[0].literal_value<int>() = equation_index;
        for (int i = 0; i < ode_count_cnexp_scalar(0); i++) {
    ...
    static void ode_update_stiff_cnexp_scalar(_nrn_mechanism_cache_range& _lmc, cnexp_scalar_Instance& inst, cnexp_scalar_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt) {
        auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
        inst.Dx[id] = inst.Dx[id] / (1.0 - nt->_dt * ( -1.0));
    }


    static void ode_setup_stiff_cnexp_scalar(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        _nrn_mechanism_cache_range _lmc{_sorted_token, *nt, *_ml_arg, _type};
        auto inst = make_instance_cnexp_scalar(&_lmc);
        auto nodecount = _ml_arg->nodecount;
        auto node_data = make_node_data_cnexp_scalar(*nt, *_ml_arg);
        ...
        for (int id = 0; id < nodecount; id++) {
            auto* _ppvar = _ml_arg->pdata[id];
            auto v = node_data.node_voltages ? node_data.node_voltages[node_data.nodeindices[id]] : 0.0;
            ode_update_stiff_cnexp_scalar(_lmc, inst, node_data, id, _ppvar, _thread, nt);
        }
    }
    ...
    extern "C" void _cnexp_scalar_reg() {
        ...
        hoc_register_cvode(mech_type, ode_count_cnexp_scalar, ode_setup_tolerances_cnexp_scalar, ode_setup_nonstiff_cnexp_scalar, ode_setup_stiff_cnexp_scalar);
