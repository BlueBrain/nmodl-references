...
namespace neuron {
    ...
    static int ode_spec1_cnexp_scalar(_nrn_mechanism_cache_range& _lmc, cnexp_scalar_Instance& inst, cnexp_scalar_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt) {
    ...
    static void ode_spec_cnexp_scalar(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            auto v = node_data.node_voltages[node_id];
            ode_spec1_cnexp_scalar(_lmc, inst, node_data, id, _ppvar, _thread, nt);
        }
    }
    ...
    static void ode_matsol_instance1_cnexp_scalar(_nrn_mechanism_cache_range& _lmc, cnexp_scalar_Instance& inst, cnexp_scalar_NodeData& node_data, size_t id, Datum* _ppvar, Datum* _thread, NrnThread* nt) {
        inst.Dx[id] = inst.Dx[id] / (1.0 - nt->_dt * ( -1.0));
    }


    static void ode_matsol_cnexp_scalar(const _nrn_model_sorted_token& _sorted_token, NrnThread* nt, Memb_list* _ml_arg, int _type) {
        _nrn_mechanism_cache_range _lmc{_sorted_token, *nt, *_ml_arg, _type};
        auto inst = make_instance_cnexp_scalar(_lmc);
        auto nodecount = _ml_arg->nodecount;
        auto node_data = make_node_data_cnexp_scalar(*nt, *_ml_arg);
        ...
        for (int id = 0; id < nodecount; id++) {
            ...
            auto* _ppvar = _ml_arg->pdata[id];
            auto v = node_data.node_voltages[node_id];
            ode_matsol_instance1_cnexp_scalar(_lmc, inst, node_data, id, _ppvar, _thread, nt);
        }
    }
