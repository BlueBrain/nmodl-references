...
namespace coreneuron {
    ...
    void nrn_init_style_ion(NrnThread* nt, Memb_list* ml, int type) {
        ...
        if (_nrn_skip_initmodel == 0) {
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                inst->ion_eca[indexes[2*pnodecount + id]] = inst->eca[id];
                int ca_type = inst->global->ca_type;
                nrn_wrote_conc(ca_type, &(inst->ion_cai[indexes[1*pnodecount + id]]), 1, inst->style_ca[4], nrn_ion_global_map, celsius, nt->_ml_list[ca_type]->_nodecount_padded);
            }
        }
    }
    ...
    void _style_ion_reg() {
        ...
        style_ion_global.na_type = nrn_get_mechtype("na_ion");
